...
- **Local dependence.** Direct effects between categorical pairs
  ...
  a rank-d eigendecomposition of the conditional Burt matrix.
...
- **A workflow for LCA**: naive fit → BVR diagnostics → direct effects →
  SLD.
- **Concomitant predictors and distal outcomes**: covariates (character
  vector and formula),
  [`predict()`](https://rdrr.io/r/stats/predict.html) semantics, BCH
  distal estimation.
- **SLD theory and worked example**: the math (conditional Burt matrix,
  hollow projector, identification, EM step) plus a synthetic example
  where SLD outperforms naive LCA.

## Citation

The methodological paper for SLD is in preparation. For now, please cite
the package:
